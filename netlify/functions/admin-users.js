...
export async function handler(event) {
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      const users = authUsers
        .map(user => {
          ...
          const usernameAliases = parseUsernameAliases(user.user_metadata?.username_aliases, username)
    ...
    if (action === 'create_user') {
      ...
      const usernameAliases = parseUsernameAliases(body.usernameAliases, username)
      ...
      const { data: created, error: createError } = await admin.auth.admin.createUser({
        ...
        user_metadata: {
          ...
          ...(username ? { username } : {}),
          ...(usernameAliases.length > 0 ? { username_aliases: usernameAliases } : {}),
    ...
    if (action === 'update_login_identifiers') {
      const userId = String(body.userId || '')
      const rawPrimary = String(body.username || '').trim()
      if (!userId) return json(400, { error: 'userId is required' })

      const primaryUsername = normalizeUsername(rawPrimary)
      const usernameAliases = parseUsernameAliases(body.usernameAliases, primaryUsername)
      ...
      const currentInternalEmail = String(targetUser.email || '').toLowerCase()
      const isUsernameAccount = currentInternalEmail.endsWith('@login.local')
      const existingUsername = normalizeUsername(targetUser.user_metadata?.username || '')

      const nextPrimaryUsername = primaryUsername || existingUsername || (isUsernameAccount ? currentInternalEmail.replace('@login.local', '') : '')
      if (!nextPrimaryUsername) {
        return json(400, { error: 'Primary username is required' })
      }

      const nextInternalEmail = isUsernameAccount
        ? `${nextPrimaryUsername}@login.local`
        : currentInternalEmail

      const { error } = await admin.auth.admin.updateUserById(userId, {
        ...(nextInternalEmail !== currentInternalEmail ? { email: nextInternalEmail } : {}),
        user_metadata: {
          ...(targetUser.user_metadata || {}),
          username: nextPrimaryUsername,
          username_aliases: usernameAliases,
          username_autogenerated: false,
        },
      })
      ...
      return json(200, {
        ...
        user: {
          ...
          username: nextPrimaryUsername,
          usernameAliases,
          internalEmail: nextInternalEmail,
        },
      })
    }
